...
document.querySelector('button.add_new').addEventListener('click', function(e){
    ...
    if (name && price && count) {
        ...
        let good_index = goods[goods.length - 1][0]
        good_index = Number(good_index.slice(5))
        goods.push(['good_'+ (good_index + 1), name, price, count, 0, 0, 0])
        ...
        for (let i = 0; i < goods.length; i++) {
            ...
            `
                <tr class="align-middle">
                    ...
                    <td>${goods[i][3]}</td>
                    <td><button class="good_delete btn btn-outline-danger" data-delete="${goods[i][0]}">&#10006;</button></td>
                    <td><button class="good_delete btn btn-outline-success" data-goods="${goods[i][0]}">&#10146;</button></td>
                </tr>
            `
            ...
            if (goods[i][4]>0) {
                ...
                <tr class="align-middle">
                    ...
                    <td class="price_discount"><input class="form-control" data-goodid="${goods[i][0]}" type="text" value="${goods[i][5]}" min="0" max="100"></td>
                    <td>${goods[i][6]}</td>
                    <td><button class="good_delete btn btn-outline-danger" data-delete="${goods[i][0]}">&#10006;</button></td>
                </tr>
                `
...
document.querySelector('.list').addEventListener('click', function(e){
    ...
    Swal.fire({
        title: 'Внимание!',
        text: 'Вы действительно хотите удалитьтовар?',
        icon: 'warning',
